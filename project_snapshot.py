...
def collect_files(
    root: Path, skip_dirs: set, only_exts: set, max_kb: int
) -> list[Path]:
    collected = []
...
def format_file_block(
    path: Path, root: Path, content: str, truncated: bool, max_lines: int
) -> str:
...
def main():
    ...
    if not files:
        ...
        for fpath in files:
            ...
            parts.append(
                format_file_block(fpath, root, content, truncated, args.max_lines)
            )
